...
--------------
		...
		local tebBuffRemain = getBuffRemain("player",_TigereyeBrew)
--------------------------------------------------
...
-----------------
	...
	-----------------------------
			...
			-- Trinkets
					if canTrinket(13) and useCDs() and tebBuffRemain > 12 then
						RunMacroText("/use 13")
					...
					if canTrinket(14) and useCDs() and tebBuffRemain > 12 then
						RunMacroText("/use 14")
						if IsAoEPending() then
							local X,Y,Z = ObjectPosition(Unit)
							CastAtPosition(X,Y,Z)
	...
	-- Burst
				if  serRemain==0 and getCombatTime() < 7 then
					if  dynamicDist.dyn40AoE<40 then
						if castSpell("player",_ChiWave,false,false) then return end
					end
					if chiDiff>=2 and cbCharge>=1  then
						if castSpell("player",_ChiBrew,false,false,false) then return end
					end
					if (chiDiff>=2 or chi==0) and power>=45 then
						if castSpell(dynamicUnit.dyn5,_Jab,false,false) then return end
					end
					if   tpRemain>fofChanTime and rskRemain>fofChanTime and tebBuffRemain > 0  then
						if castSpell(dynamicUnit.dyn5,_FistsOfFury,false,false) then return end
					end
					if getTalent(7,2) and chi>=3 and tpRemain>1 and rskRemain>1 then
	...
	-- Serenity
				if getTalent(7,3) and dynamicDist.dyn5<5 and chi>=2 and tpRemain>0 and rskRemain>0 and getCombatTime() > 7 and tebBuffRemain > 9 then
	...
	--------------------------------
				if useAoE() then
	-- Chi Explosion
					if getTalent(7,2) and chi>=4 and fofCD>3 then
						if castSpell(dynamicUnit.dyn30,_ChiExplosion,false,false) then return end
					end
	-- Rushing Jade Wind
	...
	-- Fists of Fury
					if  ttm>fofChanTime and tpRemain>fofChanTime and rskRemain>fofChanTime and serRemain==0 and getCombatTime() > 7 then
						if castSpell(dynamicUnit.dyn5,_FistsOfFury,false,false) then return end
					end
	-- Touch of Death
	...
	-- Spinning Crane Kick
					if not getTalent(6,1) and power>=40 and #getEnemies("player",8) >= 5 then
	...
	-----------------------------------
				if not useAoE() then
	-- Chi Explosion
					if getTalent(7,2) and chi>=4 and #getEnemies("target",8) >= 2 and fofCD>3 then
						if castSpell(dynamicUnit.dyn30,_ChiExplosion,false,false) then return end
					end
	-- Fists of Fury
					if ttm>fofChanTime and tpRemain>fofChanTime and rskRemain>fofChanTime and serRemain==0 and getCombatTime() > 7 then
	...
	-- Chi Explosion
					if getTalent(7,2) and chi>=3 and dynamicDist.dyn30<30 and fofCD>3 and #getEnemies("target",8) < 2 then
						if castSpell(dynamicUnit.dyn30,_ChiExplosion,false,false) then return end
					end
	-- Jab
